...
var Promise_try = Promise.try?.bind(Promise) ?? function try_(func, ...arg) {
	return new Promise((resolve) => resolve(func(...arg)));
};
...
export var saveFileStream = resourceActor({
	acquire: async (options, cancelSignal) => {
		{
			let spuriousTimeoutDeadline = performance.now() + 4;
			try {
				var handle = await showSaveFilePicker(options);
			} catch (error) {
				if (
					performance.now() < spuriousTimeoutDeadline
					&& (
						error.__proto__.constructor.name == "DOMException"
						&& error.code === DOMException.ABORT_ERR
					)
				) {
					cancelSignal.throwIfAborted();

					console.warn("https://issues.chromium.org/issues/40175286\n%o", error);
					debugger;
					handle = await showSaveFilePicker(Object.assign({ _usePolyfill: true }, options));
				} else
					throw(error);
			}
		}
